...
>$$
>Notice that if $H \leq \text{Aut}\ E$ is a [[Algebraic groups|subgroup]] generated by a set $X$, then we have $E^H = E^X$, and hence for a single element $\sigma \in \text{Aut}\ E$ we can also identify $E^\sigma =  E^{\langle \sigma \rangle}$.
...
Galois groups are related to fixed fields as follows:
- Given a subgroup $H \leq \text{Gal} (E / F)$, we have the **fixed field** $$ E^H = \{ \alpha \in E \ : \ \sigma(\alpha) = \alpha \text{ for all } \sigma \in H \} \leq E. $$ By definition, $F \leq E^H$ for every $H$. This construction is *order-reversing*, meaning that if $H \leq H’ \leq \text{Gal} (E / F)$, then $$ F = E^{\text{Gal} (E / F)} \leq E^{H'} \leq E^H \leq E = E^{\text{id}_E}. $$
- From the definitions, we also see that given an *intermediate field* $K$ such that $F \leq K \leq E$, we have a subgroup $\text{Gal} (E / K) \leq \text{Gal} (E / F)$, and given a *subgroup* $H \leq \text{Gal} (E / F)$, we have a fixed field $E^H$ such that $F \leq E^H \leq E$. The definitions alone imply the inclusions $$ H \leq \text{Gal} (E / E^H), \quad \quad K \leq E^{\text{Gal} (E / K)}. $$
