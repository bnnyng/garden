...
lastmod: 2025-08-09T00:30:13-04:00
---
# FTC I
...
$\quad$First, we show $F$ is uniformly continuous. Since $f$ is Darboux integrable, it is also bounded on $[a, b]$, and there exists $M > 0$ such that $|f(t)| \leq M$ for all $t \in [a, b]$. For all $\epsilon > 0$, set $\delta = \epsilon / M$. Then for all $a \leq x \leq y \leq b$, we have
...
$\quad$Now suppose $f$ is continuous at $x_0 \in [a,b]$, and let $\epsilon > 0$. Then there exists some $\delta > 0$ such that 
...
Since $\epsilon > 0$ is arbitrary, it follows from taking the limit as $t \to x_0$ that $F’(x_0) = f(x_0)$ as claimed. $\quad \square$
...
$\quad$ Let $\epsilon > 0$. Since $f$ is Darboux integrable, there exists a partition $P = \{ x_0, \ldots, x_n\}$ of $[a,b]$ such that the [[Development of the Darboux integral|upper and lower sums]] satisfy
...
as well. The equality follows since $\epsilon > 0$ is arbitrary. $\quad \square$