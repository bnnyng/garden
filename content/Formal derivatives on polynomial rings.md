...
lastmod: 2025-03-30T18:45:27-04:00
---
# Overview and basic definition

>[!example] Definition: Formal derivative
...
>[!abstract] Proposition: Properties of the formal derivative
...
>[!abstract] Proposition: Kernel of the formal derivative
...
💬 *Remark.* $\ \text{ }$ The statement for the case $\textup{char}F = p$ is equivalent to saying that $f = \sum_{i=0}^na_ix^i$ has $Df = 0$ iff for all indices $i$ corresponding to nonzero coefficients $a_i \neq 0$, $p$ [[Factorization on integral domains|divides]] $i$. Alternatively, $f$ is of the form $g(x^p)$ for some $$ g = \sum_{j=0}^m b_j x^j = \sum_{j=0}^m a_{jp} x^{jp}. $$
...
>[!abstract] Proposition: Criterion for an element to be a multiple root
...
- If $m \geq 2$, then we evaluate $Df(\alpha) = 0$. Since $\alpha$ is a root of $f$, we conclude $f(\alpha) = D(\alpha) = 0$ as claimed. $\ \square$

>[!abstract] Proposition: Criterion for the *existence* of a multiple root
...
>[!abstract] Corollary: Existence of multiple roots for irreducible polynomials
