...
lastmod: 2025-05-07T13:22:45-04:00
---
...
$\quad$ Formal derivatives are useful for detecting when a polynomial has [[Polynomial roots|repeated roots]]. A given root of a polynomial $f$ is repeated if it is *also* a root of its derivative $Df$. On the other hand, in a result analogous to [[(Theorem) Every nonconstant polynomial over a field has a root in some extension field]], we can also say $f$ has a repeated root in *some* [[Extension fields|extension field]] if it shares a factor (i.e., a nonconstant polynomial) with its derivative $Df$. In particular, if $f$ is [[Irreducible polynomials|irreducible]], then we require $Df = 0$.
...
>Let $D : F[x] \to F[x]$ be the $F$-linear map corresponding to the formal derivative, which has kernel defined by $\ker D = \{ f \in F[x] \ : \ Df = 0 \}$. Then depending on the [[Characteristics of rings|characteristic]] of $F$, we have 
>$$ 
>\ker D = \begin{cases} F , & \textup{if char}\ F = 0 \\ F[x^p] & \textup{if char}\ F = p > 0\end{cases}, 
>$$ 
>where $$F[x^p] = \left \{ \sum_{i=0}^n a_i x^{ip} \ : \ a_i \in F \right \} \leq F[x] $$ is the subring of all polynomials in $x^p$.
...
*Proof from [[MATH-GU4042|Modern Algebra II]].* $\ \textup{ }$ We will focus on showing that $m \geq 2$ if and only if the claimed equality holds. Suppose $\alpha \in F$ is a root of $f$, meaning $f = (x - \alpha)^m g$ for some $g \neq 0\in F[x]$. We can check each possible value of $m$:
...
- If $m \geq 2$, then we evaluate $Df(\alpha) = 0$. Since $\alpha$ is a root of $f$, we conclude $f(\alpha) = Df(\alpha) = 0$ as claimed.

$\quad$ Thus we have shown that $\alpha \in F$ is a multiple root of $f$ if and only if $m \geq 2$, if and only if $f(\alpha) = Df(\alpha) = 0$ as claimed. $\quad \square$
...
$\quad$ Assuming the claim, we have:
- $\implies$: Suppose $E$ is an extension field of $F$ and $\alpha$ is a multiple root of $f$ in $E$. Then the previous result implies $$ f(\alpha) = Df(\alpha) = 0, $$so $\alpha$ is a root of both $f$ and $Df$ and $x-\alpha$ divides both $f$ and $Df$. This means explicitly that $f, Df$ are not relatively prime in $E[x]$, so they also are not relatively prime in $F[x]$.
- $\impliedby$: Suppose $f, Df$ are not relatively prime in $F[x]$, so there exists a common *nonconstant* factor $g \in F[x]$ that divides both $f$ and $Df$. By [[(Theorem) Every nonconstant polynomial over a field has a root in some extension field]], there is an extension $E$ of $F$ that contains a root $\alpha$ of $g$. Then $\alpha$ is a common root of $f$ and $Df$ in $E$, i.e., $f(\alpha) = Df(\alpha) = 0$, so $\alpha$ is a multiple root of $f$ by the criteria above. $\quad \square$

>[!corollary] Existence of multiple roots for irreducible polynomials
>Let $f \in F[x]$ be an [[Irreducible polynomials|irreducible]] polynomial. Then we have the following:
>- (i) There exists an extension field $E$ of $F$ and a multiple root of $f$ in $E$ *if and only if* $Df = 0$. 
...
- $\impliedby$: If $Df = 0$, then $f$ is a $\gcd$ of both $f$ and $Df$, so $f, Df$ are not relatively prime in $F[x]$. $\quad \square$

*Proof of (i) $\implies$ (ii) from [[MATH-GU4042|Modern Algebra II]]*. $\quad$ Suppose $f = \sum a_i x^i$ so that $Df = \sum i a_i x^{i-1}$. Since $F$ is of characteristic zero, we know that $n \cdot a = 0$ if and only if $a = 0$ for all $n \in \mathbb Z$ and $a \in F$. Thus, $f \neq 0$ implies that there is some coefficient $a_i \neq 0$, hence $i a_i \neq 0$ and $Df \neq 0$. Then (i) implies that $f$ has no repeated roots in any extension. $\quad \square$
